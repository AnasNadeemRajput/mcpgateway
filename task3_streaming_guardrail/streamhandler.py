...
class MockUpstreamLLMClient(UpstreamLLMClient):
    ...
    async def stream_completion(self, prompt: str) -> AsyncIterator[str]:
        # Echo the prompt back first (word by word), THEN the fixed
        # PII-laden tail. Originally this ignored `prompt` entirely and
        # always returned identical output -- fine for a first pass, but
        # it meant no test could ever distinguish one prompt from
        # another, or verify that non-PII prompt content survives
        # redaction untouched. Echoing keeps the deterministic PII test
        # coverage (the tail) while making the mock actually respond to
        # its input.
        if prompt:
            for word in prompt.split(" "):
                # No artificial delay for echoed prompt words -- a
                # per-word sleep here makes wall-clock time scale with
                # prompt length, which breaks fixed-budget timing
                # assertions (e.g. "stream completes in under 2s")
                # for long prompts. Only the fixed canned tail below
                # simulates realistic token-generation latency.
                yield word + " "
...
async def _redacted_stream(prompt: str) -> AsyncIterator[str]:
    ...
    for safe_piece in buffer.finalize():
        if first_byte_at is None:
            # Real bug this fixes: for short streams entirely held back
            # until finalize() (anything under the ~253-char hold-back
            # window), TTFT was never logged at all -- the log call only
            # existed in the main-loop branch above. Short responses are
            # actually the common case for a typical LLM reply, so this
            # wasn't an edge case, it was silently skipping TTFT logging
            # most of the time.
            first_byte_at = time.monotonic()
            logger.info("TTFT: %.1fms", (first_byte_at - request_start) * 1000)
        yield safe_piece

...
class FallbackProvider:
    ...
    NOTE: _initialized is an INSTANCE attribute (set in __init__), not a
    class attribute. A class-level flag would mean the simulated 5%
    cold-start failure could only ever happen once for the entire life
    of the process, shared across every caller and every test -- and
    once tripped, it silently and permanently hides the cold-start path
    from anything that runs afterward. Per-instance state is what "cold
    start" is supposed to mean anyway (each fresh instance starts cold).
    """
    ...
    async def call_tool(
        ...
        *,
        ...
        if force_fail or (not self._initialized and random.random() < 0.05):
